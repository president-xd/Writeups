...
// Leak data for exploit — written during batch check
namespace leak_data {
    extern uint64_t U_raw;
    extern uint64_t V_raw;
    extern uint64_t chi0;
    extern uint64_t ka_last;
    extern uint64_t kb_last;
    extern uint64_t kc_last;
}
...
public:
  ...
  void andgate_correctness_check_manage() {
    ...
    if (party == ALICE) {
      ...
      // Store raw U, V for exploit before they are modified
      leak_data::U_raw = check_sum[0];
      leak_data::V_raw = check_sum[1];
  ...
  void andgate_correctness_check(uint64_t *ret, int thr_idx, uint32_t start,
    ...
    if (party == ALICE) {
      ...
      // Store leak data for exploit
      leak_data::ka_last = ka;
      leak_data::kb_last = kb;
      leak_data::kc_last = kc;
      leak_data::chi0 = chi[0];
      ret[thr_idx] = W;
    }
